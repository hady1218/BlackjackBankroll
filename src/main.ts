...
let currentTableCode: string | null = null
...
function getPlayerState(playerId: string): PlayerState | undefined {
...
function renderPlayers() {
  const playersList = byId<HTMLUListElement>("players-list")
...
function renderRound() {
  const roundStatusLabel = byId<HTMLSpanElement>("round-status-label")
  const betsList = byId<HTMLUListElement>("bets-list")
  const startRoundBtn = document.getElementById("start-round-btn") as HTMLButtonElement | null
  ...
  for (let i = 0; i < currentRound.bets.length; i++) {
    ...
    colorDot.style.backgroundColor = player?.color || "#888"
    li.appendChild(colorDot)

    const textSpan = document.createElement("span")
    let text = `${playerName} – main ${bet.handIndex} – mise: ${bet.amount}`
    if (bet.isDouble) text += " (Double)"
    if (bet.outcome) text += ` – résultat: ${bet.outcome}`
    textSpan.textContent = text
    li.appendChild(textSpan)

    const canActOnHand =
      currentRound.status === "betting" && !bet.outcome && player != null

    const canDouble =
      canActOnHand && !bet.isDouble && availableBalance >= bet.amount
...
function setBetOutcome(
  betIndex: number,
  outcome: "lost" | "won" | "blackjack" | "push" | "bust"
) {
  const connectionStatus = byId<HTMLParagraphElement>("connection-status")
...
function requestSplit(betIndex: number) {
  const connectionStatus = byId<HTMLParagraphElement>("connection-status")
...
function requestDouble(betIndex: number) {
  const connectionStatus = byId<HTMLParagraphElement>("connection-status")
...
function handleMessage(msg: IncomingMessage) {
  const connectionStatus = byId<HTMLParagraphElement>("connection-status")
  const tableCodeLabel = byId<HTMLSpanElement>("table-code-label")
  ...
  if (msg.type === "table_created") {
    ...
    const minInput = byId<HTMLInputElement>("min-bet-input")
    const maxInput = byId<HTMLInputElement>("max-bet-input")
    const startingInput = byId<HTMLInputElement>("starting-balance-input")
...
function connectAndCreateTable() {
  const connectionStatus = byId<HTMLParagraphElement>("connection-status")
  const dealerNameInput = byId<HTMLInputElement>("dealer-name-input")
  const minInput = byId<HTMLInputElement>("min-bet-input")
  const maxInput = byId<HTMLInputElement>("max-bet-input")
  const startingInput = byId<HTMLInputElement>("starting-balance-input")
...
function setupUI() {
  const connectCreateBtn = byId<HTMLButtonElement>("connect-create-btn")
  const startRoundBtn = byId<HTMLButtonElement>("start-round-btn")
  const resetTableBtn = byId<HTMLButtonElement>("reset-table-btn")
