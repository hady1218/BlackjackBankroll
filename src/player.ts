...
interface PlayerTableStateMessage {
  type: "table_state"
  tableCode: string
  players: PlayerState[]
...
interface PlayerWelcomeMessage {
  type: "welcome"
  message: string
}

interface PlayerErrorMessage {
  type: "error"
  message: string
}

interface PlayerKickedMessage {
  type: "kicked"
  reason: string
}

type PlayerIncomingMessage =
  | PlayerTableStateMessage
  | PlayerJoinedTableMessage
...
let playerCurrentTableCode: string | null = null
let playerId: string | null = null
let playerNameGlobal: string | null = null
...
function hasPlayerBetInCurrentRound(): boolean {
  if (!playerCurrentRound || !playerId) return false
  return playerCurrentRound.bets.some(b => b.playerId === playerId)
}

function updateBetControlsEnabled() {
  const betAmountInput = byIdPlayer<HTMLInputElement>("bet-amount-input")
  const placeBetBtn = byIdPlayer<HTMLButtonElement>("place-bet-btn")
  const clearBetBtn = byIdPlayer<HTMLButtonElement>("clear-bet-btn")
  const chipButtons = document.querySelectorAll<HTMLButtonElement>(".chip-btn")
...
function renderPlayerInfo() {
  const playerNameLabel = byIdPlayer<HTMLSpanElement>("player-name-label")
  const playerBalanceLabel = byIdPlayer<HTMLSpanElement>("player-balance-label")

  playerNameLabel.textContent = playerNameGlobal ?? "-"
  playerBalanceLabel.textContent = playerBalance !== null ? String(playerBalance) : "-"
}

function renderRoundStatus() {
  const roundStatusLabel = byIdPlayer<HTMLSpanElement>("round-status-label")
...
function handlePlayerMessage(msg: PlayerIncomingMessage) {
  const connectionStatus = byIdPlayer<HTMLParagraphElement>("connection-status")
  ...
  if (msg.type === "joined_table") {
    ...
    const connectJoinBtn = byIdPlayer<HTMLButtonElement>("connect-join-btn")
...
function connectAndJoinTable() {
  const connectionStatus = byIdPlayer<HTMLParagraphElement>("connection-status")
  const nameInput = byIdPlayer<HTMLInputElement>("player-name-input")
  const codeInput = byIdPlayer<HTMLInputElement>("table-code-input")
  ...
  playerWs = new WebSocket(`${wsProtocol}://${wsHost}`)
  ...
  playerWs.onmessage = (event: MessageEvent) => {
    try {
...
function addChipAmount(amountToAdd: number) {
  const betStatus = byIdPlayer<HTMLParagraphElement>("bet-status")
  const betAmountInput = byIdPlayer<HTMLInputElement>("bet-amount-input")
